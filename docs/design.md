...
- **Two sandbox types**, by who runs them:
  - **user** - for the human host user to work in interactively. Receives **no** host SSH keys of its
    own; if it needs your keys you can lend a specific set for a session (`ssh -A`), so they are never
    copied in.
  - **agent** (default) - for an autonomous coding agent. Its own persistent home, and **no**
    host SSH credentials of any kind. Autonomous agents belong here, not in a user sandbox.
...
- **first boot** (`~/.cs-sandbox-initialized` absent): seed the skeleton home from the image
  ...
  chown it; install the agent credentials. (No host `~/.ssh` is ever copied in - if a sandbox needs
  your keys you lend a specific set for the session with `ssh -A`.) `--repo` clones use a separate
...
(reaching every sandbox) and is where you work interactively rather than a place to run autonomous
agents.
...
- **No host private keys inside any sandbox.** Nothing at rest to leak from a sandbox's disk/volume;
  ...
  only in the gitignored per-sandbox seed and the home volume - never in the image or git.
- **User sandboxes are for you, not for agents.** A user sandbox is where the human works
  interactively; run autonomous coding agents in `agent` sandboxes, never in a user sandbox.
