...
#     build store"). scripts/record-build.sh names the store.
#
...
# The newest local build of $1 in the store, as "stamp version commit recorded".
newest_local() {
  local f v c r t best=""
  ...
  for f in "$store/status/$1"/*.json; do
    ...
    t="$(stamp "$v")"
    [ -n "$t" ] && [ -n "$c" ] || continue
    if [ -z "$best" ] || [ "$t" \> "${best%% *}" ]; then best="$t $v $c $r"; fi
  done
  printf '%s\n' "$best"
}
...
ago() {
  local then now d
  then="$(date -u -d "$1" +%s 2>/dev/null || date -u -j -f '%Y-%m-%dT%H:%M:%SZ' "$1" +%s 2>/dev/null || true)"
  [ -n "$then" ] || return 0
  now="$(date -u +%s)"
  d=$(( (now - then) / 60 ))
...
for t in $tools; do
  ...
  local_best=""
  [ "$LOCAL" = 0 ] || local_best="$(newest_local "$repo")"
