...
func writeRootfs(t *testing.T, instDir, name string) {
	t.Helper()
	idir := state.Dir(instDir, state.DefaultGroup, name)
...
func TestOrphansFindsLeftoverDataOfBothEngines(t *testing.T) {
	d, _ := orphanDeps(t, "cs-sandbox-home-boxp.default|2026-07-27 10:00:00.0 -0700 PDT\nunrelated|2026-07-27 10:00:00.0 -0700 PDT\n")
	...
	if got[0].Name != "boxfc.default" || got[0].Engine != state.Firecracker {
		t.Errorf("first orphan = %+v, want boxfc.default on firecracker", got[0])
	}
	if got[1].Name != "boxp.default" || got[1].Engine != state.Podman {
		t.Errorf("second orphan = %+v, want boxp.default on podman", got[1])
	}
...
func TestOrphansSkipsLiveSandboxes(t *testing.T) {
	d, _ := orphanDeps(t, "cs-sandbox-home-live.default\n")
...
func TestPurgeOrphanDeletesTheData(t *testing.T) {
	...
	if statOK(state.Dir(d.InstDir, state.DefaultGroup, "boxfc")) {
...
func TestOrphanLooksUpByName(t *testing.T) {
	...
	if _, ok := d.Orphan(context.Background(), "gone.default"); !ok {
