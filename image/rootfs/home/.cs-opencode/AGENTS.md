...
- `CS_OPENCODE_REMOTE.md` (+ `_STATUS`, `_OUTPUT`, `_SESSIONS`, `_FORGET`) · `cs-opencode-remote` —
  start or resume an OpenCode session on another host over SSH, kept warm between turns.
- `CS_CLAUDE_REMOTE.md` / `CS_CODEX_REMOTE.md` (+ the same companions) · the Claude and Codex
