...
- `CS_CODEX_LOCK_WAIT` (default 900s) — how long a turn waits for the per-session lock. The lock names the
...
Every turn end appends one line to `~/.cs-turns/codex.log`, on the machine the agent runs on:
...
`cs-claude-remote` family (see `CS_CLAUDE_REMOTE.md`, alongside this file in `~/.local/bin`).
